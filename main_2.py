...
def test(img_path, target_path, transform, net, device):
    ...
    with torch.no_grad():
        for data in test_loader:
            ...
            parking_result.append(predicted_list[0])
            print(predicted_list[0])
            print('-------------------------------------')
    return parking_result

def run_testing_task(cctv_path, cctv_txt_path):
    ...
    PATH = 'trained_model/model_ext2.pth'
    net.load_state_dict(torch.load(PATH))
    # accuracy = test(cctv_path, cctv_txt_path, transform, net, device)
    parking_result = test(cctv_path, cctv_txt_path, transform, net, device)
    #print("\nThe accuracy of testing is {:.3f}.".format( accuracy))
    return parking_result
